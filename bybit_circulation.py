...
# 로그인
# -------------------------------------------------
api_key = "dQIjgOvpCJjLOm4qaA"
api_secret = "efSOVo9X7BbkQeSkJVqtkvFjPod5YJDkmUHC"
...
class read(object):
    ...
    def read_add_price(self):  
        f = open("save_add_price.txt", "r")
        add_price = float(f.readline())
        f.close()
        return add_price

    def read_circul_cnt(self):  
        f = open("save_circul_cnt.txt", "r")
...
while True:
    ...
    try:
        ...
        if balances()['position_margin'] == 0:
            ...
            if after_time < datetime.datetime.now():
                ...
                if long_position()['position_margin'] > 0 and now_earning_rate < loss and before_rsi < rsi_down and before_rsi < now_rsi:
                    ...
                    if counting == 1:
                        ...
                        # 정보 저장
                        # 초기 USDT 잔고 - 기준 매수 시간 - 축적 매수량 - 평균 매수가 - 카운팅 - 추가 매수가 - 순환매 가능 여부
                        s = save(0,after_time,accumulated_volume,ap,counting+1,add_price,circul_cnt) 
                        s.save_after_time()
                        s.save_accumulated_volume()
                        s.save_avg_price()
                        s.save_counting()
                    ...
                    if counting > 1:
                        ...
                        # ------------------추가 매수분 가격 조정 필요---------------------------
                            ...
                            # 정보 저장
                            # 초기 USDT 잔고 - 기준 매수 시간 - 축적 매수량 - 평균 매수가 - 카운팅 - 추가 매수가 - 순환매 가능 여부
                            s = save(0,after_time,accumulated_volume,ap,counting+1,0,circul_cnt)
                            s.save_after_time()
                            s.save_accumulated_volume()
                            s.save_avg_price()
                            s.save_counting()
            ...
            # 롱 포지션 보유중 - RSI 꺾인 지점 70 이상
            if long_position()['position_margin'] > 0 and before_rsi > rsi_up and before_rsi > now_rsi:
                

                # 수익률 2% 이상이면 전량 매도
                if get_current_price(btc) > avg_buy_price and now_earning_rate > 2:
